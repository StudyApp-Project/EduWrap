    ...
    <div className="p-4 md:p-6 space-y-6 overflow-auto">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)] mt-1">Find a community to learn with.</p>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {CATEGORIES.map(cat => (
            <button key={cat.id} className="flex items-center gap-3 p-4 rounded-2xl border border-[var(--border-default)] transition-all hover:shadow-[var(--shadow-md)] hover:-translate-y-0.5" style={{ background: 'var(--bg-surface)' }}>
              <span className="text-2xl">{cat.icon}</span>
              <div className="text-left">
                <span className="text-sm font-medium block">{cat.label}</span>
                <span className="text-[10px] text-[var(--text-muted)]">{cat.count} rooms</span>
              </div>
            </button>
          ))}
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {ACTIVE_ROOMS.map(room => (
            <div key={room.id} className="p-4 rounded-2xl border border-[var(--border-default)] transition-all hover:shadow-[var(--shadow-md)]" style={{ background: 'var(--bg-surface)' }}>
              ...
              <p className="text-xs text-[var(--text-muted)] mb-3">{room.subject}</p>
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-1 text-xs text-[var(--text-muted)]"><Users size={12} /> {room.members}</span>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[var(--border-default)] bg-[var(--bg-glass)]">
            <Search size={14} className="text-[var(--text-muted)]" />
            <input type="text" placeholder="Find rooms..." className="bg-transparent outline-none text-xs w-32 text-[var(--text-primary)] placeholder:text-[var(--text-muted)]" />
          </div>
        </div>
        <div className="space-y-2">
          {PUBLIC_ROOMS.map(room => (
            <div key={room.id} className="flex items-center gap-4 p-4 rounded-xl border border-[var(--border-default)] transition-all hover:shadow-[var(--shadow-sm)]" style={{ background: 'var(--bg-surface)' }}>
              <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-[var(--bg-glass)] text-[var(--text-secondary)]">
                <Globe size={16} />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-medium">{room.name}</h4>
                <p className="text-[10px] text-[var(--text-muted)]">{room.subject}</p>
              </div>
              <span className="hidden sm:flex items-center gap-1 text-xs text-[var(--text-muted)]"><Users size={12} /> {room.members} members</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${room.status === 'live' ? 'bg-green-500/10 text-green-500' : room.status === 'scheduled' ? 'bg-blue-500/10 text-blue-500' : 'bg-[var(--bg-glass)] text-[var(--text-muted)]'}`}>
                {room.status}
              </span>
              <button className="px-3 py-1.5 rounded-lg text-xs font-medium border border-[var(--border-strong)] transition-all hover:bg-[var(--bg-glass)]">
                Join Room
              </button>
            </div>
          ))}

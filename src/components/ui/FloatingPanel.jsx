  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed z-50 bg-[var(--bg-elevated)] border border-[var(--border-strong)] rounded-xl shadow-[var(--shadow-lg)] overflow-hidden flex flex-col"
        style={{ x: defaultPosition.x, y: defaultPosition.y }}
      >
        <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--border-default)] bg-[var(--bg-surface)] shrink-0 touch-none cursor-move">
        ...
        <AnimatePresence>
          {!isMinimized && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="flex-1 overflow-auto p-4 bg-[var(--bg-base)]"
            >
              {children}
            </motion.div>
          )}
